...
#    FILE_NAME=$(basename $i | sed s'/.zip//'g);
    ...
    OUT_DIR=`get_html_home $DEVICE`/ResurrectionRemix/$DEVICE_NAME/$BUILD_DATE
    mkdir -p $OUT_DIR
#    F_DIR="$(dirname $i)"
...
#    FILE_NAME=$(basename $i | sed s'/.zip//'g);
    ...
    DEVICE_NAME=$(echo $FILE_NAME | cut -d '-' -f 3)
    BUILD_DATE=$(echo $FILE_NAME | cut -d '_' -f 3)
    OUT_DIR=`get_html_home $DEVICE`/LineageOS/$DEVICE_NAME/$BUILD_DATE
    mkdir -p $OUT_DIR
#    F_DIR="$(dirname $i)"
    F_DIR=${TRANSMISSION_OUT}/${FILE_NAME}
...
#    FILE_NAME=$(basename $i | sed s'/.zip//'g);
    ...
    BUILD_DATE=$(echo $FILE_NAME | cut -d '_' -f 3)
    OUT_DIR=`get_html_home $DEVICE`/LineageOS_Go/$DEVICE_NAME/$BUILD_DATE
    mkdir -p $OUT_DIR
#    F_DIR="$(dirname $i)"
    F_DIR=${TRANSMISSION_OUT}/${FILE_NAME}
